...
const { data: markets } = await trending.json();

// Get smart money signals for Hyperliquid
const signals = await client.fetch(
  "https://agent.metengine.xyz/api/v1/hl/smart-wallets/signals?timeframe=24h",
);
const { data: hlSignals } = await signals.json();

// Find trending Meteora pools
const pools = await client.fetch(
  "https://agent.metengine.xyz/api/v1/meteora/pools/trending?timeframe=24h",
);
const { data: meteoraPools } = await pools.json();
```
...
The `/.well-known/mpp` endpoint returns the complete machine-readable service manifest including all endpoints, descriptions, pricing tiers, and payment configuration. Use it for programmatic discovery.
